...
function App() {
  ...
  const [qualQuerNome,setQualQuerNome] = useState(requisicao[0].titulo)
  

  function alterarNome() {
    axios.get('https://back-filmes.herokuapp.com/filmes').then((resposta) => { console.log(resposta.data)
    setCards(resposta.data)
    setQualQuerNome(resposta.data[0].titulo)
    })
  }
  return (
    <>
    <Header title={cards[0].titulo}/>
    <button className="refresh-button" onClick={alterarNome}>Alterar nome</button>
